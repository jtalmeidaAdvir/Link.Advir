...
async function handleClienteInputIntervencao(
    ...
    try {
        ...
        const token = await getAuthToken(
            {
                username: process.env.PRIMAVERA_USERNAME || "AdvirWeb",
                password: process.env.PRIMAVERA_PASSWORD || "Advir2506##",
                company: process.env.PRIMAVERA_COMPANY || "Advir",
                instance: process.env.PRIMAVERA_INSTANCE || "DEFAULT",
                line: process.env.PRIMAVERA_LINE || "Evolution",
            },
            process.env.PRIMAVERA_API_HOST || "151.80.149.159:2018",
        );

        // Buscar todos os pedidos primeiro
        const pedidosResponse = await axios.get(
            `http://${process.env.PRIMAVERA_API_HOST || "151.80.149.159:2018"}/WebApi/ServicosTecnicos/ObterPedidos`,
            {
                headers: {
    ...
    if (pedidoSelecionado.Tecnico) {
        ...
        try {
            ...
            const tecnicosResponse = await axios.get(
                `http://${process.env.PRIMAVERA_API_HOST || "151.80.149.159:2018"}/WebApi/ServicosTecnicos/LstTecnicosTodos`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json",
    ...
    try {
        ...
        try {
            const response = await axios.post(
                `http://${process.env.PRIMAVERA_API_HOST || "151.80.149.159:2018"}/WebApi/ServicosTecnicos/CriarIntervencoes`,
                intervencaoData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
...
// Rota principal para criar intervenção via API direta

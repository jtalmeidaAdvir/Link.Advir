...
const criarVisitante = async (req, res) => {
  try {
    const { primeiroNome, ultimoNome, numeroContribuinte, empresa_id } = req.body;
    const empresaId = empresa_id || req.user?.empresa_id;

    console.log('📝 Tentando criar visitante:', { primeiroNome, ultimoNome, numeroContribuinte, empresaId });

...
router.get("/GetResponsavel/:codFuncionario", async (req, res) => {
    try {
        ...
        const apiUrl = `http://${urlempresa}/WebApi/Obras/GetResponsavel/${codigo}`; // A URL completa da API
        console.log("Enviando solicitação para a URL:", apiUrl);
        ...
        if (response.status === 200) {
            ...
            if (!pedidos || pedidos.length === 0) {
                return res
                    .status(404)
                    .json({ error: "Nenhum responsavel encontrado." });
            }
            ...
            return res
                .status(400)
                .json({
                    error: "Falha ao listar responsavel.",
                    details: response.data.ErrorMessage,
                });
        }
    } catch (error) {
        console.error("Erro ao listar responsavel:", error.message);
        return res
            .status(500)
            .json({
                error: "Erro inesperado ao listar responsavel",

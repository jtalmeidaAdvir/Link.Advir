...
router.get('/VerificaContacto/:cliente/:telefone', async (req, res) => {
    try {
        const painelAdminToken = req.headers['authorization']?.split(' ')[1];
        if (!painelAdminToken) {
            return res.status(401).json({ error: 'Token de administrador não encontrado. Faça login novamente.' });
        ...
        // Agora os parâmetros são extraídos diretamente da URL
        const { cliente, telefone } = req.params;  // Use req.params para obter os parâmetros na URL
 
        if (!cliente || !telefone) {
            return res.status(400).json({ error: 'Parâmetros não fornecidos na URL.' });
        }
 
        const apiUrl = `http://${urlempresa}/WebApi/ServicosTecnicos/VerificaContacto/${cliente}/${telefone}`;
        console.log('Enviando solicitação para a URL:', apiUrl);
...
router.get('/GetPedidosByTecnico/:tecnico', async (req, res) => {
    try {
        ...
        const apiUrl = `http://${urlempresa}/WebApi/ServicosTecnicos/GetPedidosByTecnico/${tecnico}`;
        console.log('Enviando solicitação para a URL:', apiUrl);
...
router.get('/VerificaExisteObjeto/:Objeto/:Descricao', async (req, res) => {
    try {
        const painelAdminToken = req.headers['authorization']?.split(' ')[1];
        if (!painelAdminToken) {
            return res.status(401).json({ error: 'Token de administrador não encontrado. Faça login novamente.' });
        ...
        // Agora os parâmetros são extraídos diretamente da URL
        const { Objeto, Descricao } = req.params;  // Use req.params para obter os parâmetros na URL
 
        if (!Objeto || !Descricao) {
            return res.status(400).json({ error: 'Parâmetros não fornecidos na URL.' });
        }
 
        const apiUrl = `http://${urlempresa}/WebApi/ServicosTecnicos/VerificaExisteObjeto/${Objeto}/${Descricao}`;
        console.log('Enviando solicitação para a URL:', apiUrl);
        ...
        if (response.status === 201 || response.status === 200) {
            ...
            return res.status(response.status).json({
                error: 'Falha ao obter informações de Objeto.',
                details: response.data.ErrorMessage || 'Erro desconhecido',
            });
        }
    } catch (error) {
        console.error('Erro ao obter informações de Objeto:', error.response ? error.response.data : error.message);
        return res.status(500).json({
            error: 'Erro inesperado ao obter informações de Objeto',
            details: error.message,
        });
    }
});
...
router.get('/FecharPedido/:PedidoID', async (req, res) => {
    try {
        ...
        const apiUrl = `http://${urlempresa}/WebApi/ServicosTecnicos/FecharPedido/${PedidoID}`;
        console.log('Enviando solicitação para a URL:', apiUrl);

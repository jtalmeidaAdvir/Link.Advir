...
router.get('/listarPedidos', async (req, res) => {
    try {
        ...
        if (response.status === 200) {
            ...
            return res.status(400).json({ error: 'Falha ao listar clientes.', details: response.data.ErrorMessage });
        }
    } catch (error) {
        console.error('Erro ao listar pedidos:', error.message);
        return res.status(500).json({ error: 'Erro inesperado ao listar pedidos', details: error.message });
    }
});

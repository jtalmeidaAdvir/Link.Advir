...
const API_BASE_URL = "https://backend.advir.pt/whatsapi/api/whatsapp/api/whatsapp";
    ...
    const saveEditedContacts = async (contactListData) => {
        try {
            ...
            for (const contactData of contactsToSave) {
                const response = await fetch(`https://backend.advir.pt/whatsapi/api/whatsapp/api/whatsapp/contacts`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
